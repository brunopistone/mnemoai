...
| Directory               | Role                                                                                                                                                               |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `client/`               | LangGraphClient facade, MCP bridge, session transcripts (`session_log.py`)                                                                                         |
...
**Automatic**, append-only per-directory transcripts that `mnemoai --resume` restores — the counterpart to the **user-curated** `/save`+`/load` (which stay untouched and are NEVER swept). One JSONL file per session under `{profile}/sessions/{sanitize_cwd(cwd)}/session_<ts>_<pid>_<rand>.jsonl`, so resuming in a project only offers that project's sessions. Record types: `meta` (session id, cwd, model), `turn` (the messages that turn added), `compact`. CLI: `--resume` (picker, newest-first, listing capped at 20), `--resume <id>`, `--continue` (most recent, non-interactive) — all handled in `main._resume_session` → `client.resume_session`, which reuses the SAME decode+replay path as `/load`. Expiry is **age-based only** (`SESSION_MAX_AGE_DAYS`, default 30, root-level config key; `0` disables recording entirely) and sweeps EVERY project dir at startup — the picker cap limits what's _offered_, never what's kept, so a cap can't silently drop a session.
...
Semver. The **public contract** (what a major bump protects) is: `config.yaml` keys (model sections, `ENABLE_*`/`REQUIRE_*` toggles, documented sections), the `mcp.json` `mcpServers` schema, the CLI slash-commands + `mnemoai` console command (incl. its `--no-verbose`/`--resume`/`--continue` flags), and the `mnemoai-assistant` dist / `mnemoai` import name. Everything under `client/`/`server/`/`models/`/`utils/` not in that list is internal and may change freely. All changes go in `CHANGELOG.md`; releases follow the checklist in `docs/development/index.md`. CI (`.github/workflows/tests.yml`) runs the unit tier + import-sort on push/PR; the integration tier is run locally before releases.
