...
| `client/memory/playbook_store.py`  | **ACE Playbook.** Append-only store for learned strategies with lazy semantic deduplication. Retrieves relevant entries by task context for system prompt injection. Key class: `PlaybookStore` with `append()`, `append_batch()`, `get_relevant_entries()`, `format_for_prompt()`, `_refine()` (triggers when over max_entries). Persists at `~/agent-conversations/{profile}/models/{model}/playbook/playbook.json` (model-scoped). |
