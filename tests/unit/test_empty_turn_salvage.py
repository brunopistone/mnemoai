...
class TestDegenerateRepetition:
    f = staticmethod(LangGraphAgent._is_degenerate_repetition)

    def test_detects_unused_token_wall(self):
        assert self.f("our" + "<unused6226>" * 50)

    def test_detects_whitespace_separated_tokens(self):
        assert self.f("text " + "<pad> " * 40)

    def test_ignores_short_text(self):
        assert not self.f("<unused6226><unused6226>")

    def test_ignores_prose(self):
        assert not self.f("The quick brown fox " * 30)

    def test_ignores_markdown_list(self):
        assert not self.f("- item one\n- item two\n- item three\n" * 10)

    def test_ignores_varied_tags(self):
        assert not self.f("<li>x</li><li>y</li><li>z</li>" * 20)


def test_stream_aborts_degenerate_loop_early():
    # A model spewing one reserved token must be cut off — the stream stops
    # well before consuming every chunk, and the response carries a clear note.
    ...
    class _SpewModel:
        ...
        def stream(self, messages, config=None):
            yield _Chunk(content="our")
            # Far more than the detector threshold; if not aborted, all run.
            for _ in range(500):
                self.yielded += 1
                yield _Chunk(content="<unused6226>")

    model = _SpewModel()
    resp, _ = a._stream_response(["msg"], {}, model=model, mark_answer=True)
    # Aborted long before all 500 degenerate chunks were consumed.
    assert model.yielded < 100
    assert "repetition loop" in resp.content
