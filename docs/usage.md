...
| `/plan`            | Toggle **plan mode** — an enforced read-only mode (Claude-Code-style). While ON, the agent investigates and presents a plan; **read-only** shell commands (`ls`, `cat`, `grep`, `git status/log/diff`, …) still run and it may draft the plan to a `.md` file under the plans dir, but file edits, mutating shell commands, git writes, and background tasks are **hard-blocked** until you `/plan` again to exit |
