...
| `utils/config.py`                     | **Singleton config manager.** Loads config via `_resolve_config_path()` (`$PERSONAL_AI_ASSISTANT_CONFIG` → `<app_home>/config.yaml` → repo `utils/config.yaml` fallback; prints a copy-a-template hint if none found). Exposes `.get("SECTION.KEY", default)` and `.system_prompt` property (injects current date). Sets env vars from `ENV` section.             |
