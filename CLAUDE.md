...
**No StateGraph:** Unlike the langgraph branch, there is no `agent.py` / StateGraph — the Strands `Agent` runs the model↔tool loop internally. Orchestrator-workers _is_ supported (see `client/orchestrator.py` + `StrandsClient._handle_orchestrated_query`), but it's a hand-rolled loop over Strands `Agent` instances rather than graph nodes.
