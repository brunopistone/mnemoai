...
def tool_error_handler(func: Callable) -> Callable:
    ...
    Sync in, sync out: a blocking tool is written as a plain ``def`` so
    ``thread_offload`` can run it off the server's event loop, and an async-only
    wrapper here would have forced it back to ``async def``. Both wrappers share
    ONE except-chain (:func:`_error_response`) so the two can't drift.
    ...
    if inspect.iscoroutinefunction(func):
        ...
        async def wrapper(*args, **kwargs) -> str:
            try:
                ...
                return _error_response(e, func.__name__)
...
def _error_response(exc: Exception, func_name: str) -> str:
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in {func_name}: {str(e)}", exc_info=True
        )
        return json.dumps(
            {
                "error": True,
