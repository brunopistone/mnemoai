...
| Command                    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/help`                    | Show the command reference and keyboard keys. The launch banner deliberately doesn't print it — this is where the full list lives, and `/` searches it as you type                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `/config`                  | Re-run the interactive configurator — chat, vision (with a "same as chat?" shortcut + provider choice), and embeddings models, then feature toggles, then a model for each enabled internal call (router, orchestrator). `Ctrl+B` steps back within a model section. Overwrites `config.yaml` and restarts in place                                                                                                                                                                                                                                                      |
| `/model`                   | Set up one model — chat (LLM), vision, embeddings, or one of the internal calls (router, orchestrator, summary — see [per-area models](../configuration.md#per-area-models-area_models)). Vision and the internal calls offer a "use the same model as Chat?" shortcut. Inference params reset to model defaults on a change (re-tune with `/params`). Chat, vision and embeddings restart in place; an internal call is **applied without restarting**, so the current conversation continues (unless the same edit switches routing or orchestration on, which needs the restart)                                                                                                                                                                                                  |
...
| `/auto [tier]`             | **Skip the confirmation prompt** for a tier of operations, this session only — `off` → `edits` (writes inside the working directory) → `writes` (any path, plus memory) → `all` (also shell). Bare `/auto` steps to the next tier; the input prompt carries a colored badge while one is active. Launch with `mnemoai --auto` to start at the top tier. The safety floor, plan mode, hook denials and git safety overrides are unaffected ([details](safety.md#stop-being-asked-for-a-while))                                                                           |
