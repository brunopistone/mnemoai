...
| `/model`                   | Set up one model — chat (LLM), vision, embeddings, or one of the internal calls (router, orchestrator, summary — see [per-area models](../configuration.md#per-area-models-area_models)). Vision and the internal calls offer a "use the same model as Chat?" shortcut. Inference params reset to model defaults on a change (re-tune with `/params`); restarts in place                                                                                                                                                                                                  |
